...
import { Department, Shift,Skill } from "./Models/DisplayReactComponentModels";
...
export const WorkforceDisplay: React.FC<IWorkforceDisplayProps> = ({
  ...
  const ShiftModalInline = React.useCallback(({
    isOpen,
    selectedWorker,
    selectedShift,
    departmentData,
    ...
    useEffect(() => {
     ...
     const getSkillsForDepartment = (departmentId: string) => {
        const department = departmentData.find(
          (dept :Department) => dept.Departementid === departmentId
        );
      ...
      const handleConfirmClick = () => {
        ...
        const selectedDepartmentData = departmentData.find(
          (dept : Department) => dept.Departementid === selectedDepartment
        );
        ...
        if (isCreateMode &&  onCreateShift) {
          const shiftDate = selectedDate || (selectedShift?.ava_shiftday ? new Date(selectedShift.ava_shiftday) : new Date());
      ...
      const modalTitle = isRecreatingShift 
        ? "Recreate Shift" 
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="form-group">
              ...
              >
                ...
                {getSkillsForDepartment(selectedDepartment).map((skill: Skill) => (
    ...
    }, []); // Empty dependency array since we're using props
